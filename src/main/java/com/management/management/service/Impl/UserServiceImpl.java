...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public GenericResponse<?> getUserByEmail(@Valid String email) {
        try {
    ...
    @Override
    public GenericResponse<?> getCurrentUser(String email) {
        try {
            User user = userRepository.findByEmail(email);
            if (user == null) {
                return new GenericResponse<>(HttpConstants.FAILURE, "User not found", null);
            }
            user.getBooks().forEach(book -> book.setQuantity(1));
            return new GenericResponse<>(HttpConstants.SUCCESS, "User retrieved successfully", user);
        } catch (Exception e) {
            return new GenericResponse<>(HttpConstants.FAILURE, "Error retrieving user: " + e.getMessage(), null);
        }
    }
}